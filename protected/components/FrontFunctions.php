...
class FrontFunctions
{
	...
	public static function getLogoURL()
	{
		...
		if(!empty($logo)){
			...
			} else return Yii::app()->getBaseUrl(true)."/assets/images-front/logo.png";
		} else return Yii::app()->getBaseUrl(true)."/assets/images-front/logo.png";
	}
	...
	public static function getMenu($menu_id='')
	{
		...
		$cmenu[]=array('visible'=>true,'label'=>t('Home'),
	        'url'=>array('/front/index'),'linkOptions'=>array());
	        
	    $cmenu[]=array('visible'=>true,'label'=>t('Pricing'),
	        'url'=>array('/front/pricing'),'linkOptions'=>array());    
	    ...
	    $cmenu[]=array('visible'=>true,'label'=>t('login in'),	 
		        ...
		        'url'=>array('/app/login'),'linkOptions'=>array(
		          'class'=>"login"
		        ));
		} else {
	...
	public static function getPlans($exclude_free=false)
	{
		$and='';
		if ($exclude_free){
			$and=" AND price>0";
	...
	{		
		if (isset($data['email_address'])){
            ...
            $email_admin=getOptionA('email_admin');
			...
			if(!empty($global_sender)){
			   if(sendEmail($data['email_address'],$global_sender,$subject,$tpl)){
                   if(!empty($email_admin)){
                       sendEmail(
                           $email_admin,
                           $global_sender,
                           'Signup needs approval',
                           $data['first_name']. ' ' . $data['last_name'] .' already register, please review for approval.'
                       );
                   }
               }
			}
	...
	public static function formatPricing($price='')
	{
		...
		} else $hml="<price><span>$curr</span> $final_price</price>";
		return $hml;
	}	
	...
	public static function savePaymentLogs($customer_id,
	  $transaction_type='',
	  $payment_provider='',
	  $memo='',
	  $total_paid=0,
	...
	public static function getTaskView($idToken='',$idTask='')
    {
        if(!empty($idToken) && !empty($idTask)){
            $db=new DbExt;
            ...
            {{driver_task_view}}
            WHERE
            task_id=".self::q($idTask)."
            AND task_token=".self::q($idToken)."		
            LIMIT 0,1
            ";
            if ($res=$db->rst($stmt)){
                return $res[0];
    ...
    public static function distance($origin,$destination,$mode)
    {
        ...
        $api = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=".urlencode($origin)."&destinations=".urlencode($destination)."&mode=".urlencode($mode)."&sensor=false&key=".urlencode($google_key);
    ...
    public static  function getSpeed($lat1,$lng1,$lat2,$lng2)
    {
        $R = 6371; // km
