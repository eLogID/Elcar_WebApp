...
{	
	...
	{
		if(isset($_SESSION['elcar_admin'])){
			if(is_numeric($_SESSION['elcar_admin']['admin_id'])){
				return true;
			}
		}
		return false;
	...
	public static function getAdminID()
	{
		if(isset($_SESSION['elcar_admin'])){
			if(is_numeric($_SESSION['elcar_admin']['admin_id'])){
				return $_SESSION['elcar_admin']['admin_id'];
			}
		}
		return false;
	}
	...
	public static function getAdminUsername()
	{
		if(isset($_SESSION['elcar_admin'])){
			if(is_numeric($_SESSION['elcar_admin']['admin_id'])){
				return $_SESSION['elcar_admin']['first_name'];
			}
		}
		return false;
	}
	...
	public static function prettyPrice($price='' , $currency_code='')
	{
		...
		} else $output=$curr.$spacer.$final_price;
		return $output;
	}
	...
	public static function generateActionsList($where_field='',$where_val='',$table='',$slug='')
	{
		...
		$html='
         ...
         <a title="'.t("Delete").'" href="javascript:;" class="rm-records btn-floating btn-small red lighten-1" 
	...
	{		
		$html='
		 <a title="'.t("Approved").'" href="javascript:;" data-id="'.$id.'" class="approved-customer btn-floating btn-small teal lighten-1" style="margin-right:10px;">
    ...
    public static function generateActionsView($user='',$pass='')
	{
        $link=Yii::app()->createUrl('app/login',array(
            'a'=>$user,
            'q'=>$pass
        ));

		$html='
		 <a title="'.t("View").'" href="'.$link.'" target="_blank" class="btn-floating btn-small teal lighten-1">
          <i class="material-icons tiny">view</i>
         </a>                  
         ';
		return $html;
	}
    ...
    public static function getTotalSignup($start='', $end='')
    {
		...
		if($res=$db->rst($stmt)){
			...
			$first_date=date("Y-m-d",strtotime($res[0]['date_created']."-1 day"));
				$data[]=array(
				   'date'=>$first_date,
				   'total'=>0				   
		    );
	...
	public static function getCustomerPaymentLogs($customer_id='',$limit=50)
	{
		$db=new DbExt;
    	$stmt="SELECT * FROM
    	{{payment_logs}}
