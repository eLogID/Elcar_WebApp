...
class AjaxfrontController extends CController
{
	...
	{		
		...
		if (isset($this->data['plan_id'])){
            ...
            $params['password_decrypt']=$params['password'];
	...
	public function actionrunTrackMap()
    {
        if (isset($this->data['id'])){
            if (isset($this->data['task_id'])){
                if($res=FrontFunctions::getTaskView($this->data['id'],$this->data['task_id'])){
                    $this->code=1;
                    $this->msg=t("OK");
                    ...
                    if(!empty($res['photo_profile'])){
                        $res['photo_name_url']=Driver::uploadURL()."/photo/".$res['photo_profile'];
                        if (!file_exists(Driver::uploadPath()."/photo/".$res['photo_profile'])){
                            $res['photo_name_url']=Driver::assetsUrl()."/images/no-avatar.jpg";
                        }
                    }
                    ...
                    $origin=$res['driver_location_lat'].','.$res['driver_location_lng'];
                    $destination=$res['task_lat'].','.$res['task_lng'];

                    $res['origin']=$origin;
                    $res['destination']=$destination;

                    $distance=FrontFunctions::distance($origin,$destination,$mode);
                    if(!empty($distance)){
                        if($distance->{'status'}=="OK"){
                            $eta = $distance->{'rows'}[0]->{'elements'}[0]->{'duration'}->{'text'};
                            $mileage = $distance->{'rows'}[0]->{'elements'}[0]->{'distance'}->{'text'};
                        }
                    }
                    $res['distance']=$distance;

                    $res['eta']=$eta;
                    $res['find_driver_label']='Find Agent';
                    $res['driver_info_window']='<p>'.$res['driver_name'].'</p><p>Your Agent</p>';
                    $res['travel_mode']=$mode;
                    $res['drofoff_info_window']='<p></p><p>Pickup Details</p>';
                    $res['track_details']='<div class="row">
                                             <div class="col-xs-1">
                                                <div class="avatar-wrapper">
                                                    <img src="'.$res['photo_name_url'].'" class="avatar">
                                                    <div class="status-circle cicle-offline"></div>
                                                </div> <!--avatar-wrapper-->
                                             </div>
                                             <div class="col-xs-4" style="padding-left:0;">
                                                <p class="agent-name">'.$res['driver_name'].'<br/>
                                                   <span>Your Agent</span><br/>
                                                   <span style="font-size:11px;">Task ID: '.$res['task_id'].'</span>
                                                </p>
                                             </div>
                                             <div class="col-xs-7 text-right">
                                                <div class="eta-wrap">
                                                   <p>ETA<br/>
                                                      <b class="track_eta">Distance:'. $mileage .' | Duration:'.$eta.'</b><br/>
                                                      <span class="task-status">Task Status : 
                                                         <b class="task-stats"><span class="tag assigned">'.$res['status'].'</span></b>
                                                      </span>
                                                   </p>
                                                </div> <!--eta-wrap-->
                                                <div class="track-contact-wrap">
                                                   <a href="tel:'.$res['driver_phone'].'" ><i class="ion-ios-telephone-outline"></i></a>
                                                </div>
                                             </div> <!--text-right-->
                                           </div> <!--row-->';
                    $res['contact_phone']='<a href="tel:'.$res['driver_phone'].'" ><i class="ion-ios-telephone-outline"></i></a>';
            ...
            } else $this->msg=t("Task id is missing");
        } else $this->msg=t("Token is missing");
        $this->jsonResponse();
    }
}/* end class*/