...
class AjaxController extends CController
{
    ...
    public function actionLogin()
    {
        ...
        if($Validator->validate()){
            ...
            if ( $res = Driver::Login(trim($this->data['email_address']), trim($password), $q )){
    ...
    public function actionviewerList()
    {
        ...
        if ( $res=$DbExt->rst($stmt)){
            ...
            foreach ($res as $val) {
                ...
                $id=$val['viewer_id'];
                $p="id=$id"."&tbl=viewer&whereid=viewer_id";

                $actions="<div class=\"table-action\">";
                $actions.="<a data-modal=\".new-viewer\" data-id=\"$id\" 
			    data-action=\"getViewerInfo\"
			    class=\"table-edit\" href=\"javascript:;\">".Driver::t("Edit")."</a>";
    ...
    public function actionaddViewer()
    {
        $DbExt=new DbExt;
        $params=array(
        ...
        if(is_numeric($this->data['id'])){
            ...
            if(Driver::checkViewerUserExist($this->data['username'],$this->data['id'])){
                $this->msg=t("Username already exist");
                $this->jsonResponse();
                Yii::app()->end();
            }

            if ( $DbExt->updateData("{{viewer}}",$params,'viewer_id',$this->data['id'])){
                $this->code=1;
                $this->msg=Driver::t("Successfully updated");
                $this->details='new-viewer';

            } else $this->msg=Driver::t("failed cannot update record");

        } else {

            if(Driver::checkViewerUserExist($this->data['username'])){
                $this->msg=t("Username already exist");
                $this->jsonResponse();
                Yii::app()->end();
            }

            if ( $DbExt->insertData('{{viewer}}',$params)){
                $this->code=1;
                $this->msg=Driver::t("Successful");
                $this->details='new-viewer';
            } else $this->msg=Driver::t("failed cannot insert record");
    ...
    public function actiongetViewerInfo()
    {
        if(isset($this->data['id'])){
            if ( $res=Driver::viewerInfo($this->data['id'])){
                $this->code=1;
                $this->msg=Driver::t("Successful");
                $this->details=$res;
            } else $this->msg=Driver::t("Record not found");
        } else $this->msg=Driver::t("Missing parameters").$this->data['id'];
        $this->jsonResponse();
    }
    ...
    public function actionAddTask()
    {
        ...
        if($Validator->validate()){
            ...
            $params=array(
                ...
                'reff'=>isset($this->data['reff'])?$this->data['reff']:''
            );
            ...
            foreach ($agent_stats as $agent_stat) {
                ...
                if (is_array($res_agent) && count($res_agent)>=1){
                    foreach ($res_agent as $agent_val) {
                        ...
                        if (!empty($agent_val['photo_profile'])){
                            $photo_url=Driver::uploadURL()."/photo/".rawurldecode($agent_val['photo_profile']);
                            if (!file_exists(Driver::uploadPath()."/photo/".rawurldecode($agent_val['photo_profile']))){
                                $photo_url=Yii::app()->getBaseUrl(true).'/assets/images/no-avatar.jpg';
                            }
                        }
                        $coordinates[]=array(
                            ...
                            'photo_url'=>$photo_url,
                            'device_platform'=>$agent_val['device_platform'],
                            'transport_description'=>$agent_val['transport_description'],
                            'licence_plat'=>$agent_val['licence_plat']
                        );
                    }
                }
            }
    ...
    public function actionGetTaskDetails()
    {
        ...
        if (isset($this->data['id'])){
            if ( $res=Driver::getTaskId($this->data['id'])){
                ...
                $res['delivery_date']=!empty($res['delivery_date'])?date("Y-m-d G:i",strtotime($res['delivery_date'])):'-';
                $res['trans_type']=!empty($res['trans_type'])?$res['trans_type']:'';
                $res['task_token_url']=!empty($res['task_token'])?Yii::app()->getBaseUrl(true)."/track/?id=".$res['task_token'].'&task_id='.$res['task_id']:'';
                ...
                if($info=$res){
                    if($history_details = Driver::getTaskHistory($this->data['id'])){
                        $datetime = strtotime($info['date_created']);
                        $starttask='';
                        foreach ($history_details as $valh) {
                            ...
                            if(!empty($valh['photo_name'])){
                                ...
                                if (!file_exists(Driver::uploadPath()."/photo/".$valh['photo_name'])){
                                    $valh['photo_name_url']='';
                                }
                            }
                            ...
                            /* get diff time */
                            $betweentime='';
                            if($valh['status']=="notes" || $valh['status']=="photo" || $valh['status']=="sign" ){
                                //nothing to do..
                            } else {
                                if($valh['status']=="started"){
                    ...
                    if($trip_details = Driver::getTaskTrip($this->data['id'])){
                        foreach ($trip_details as $valt) {
                            $valt['date_created']=Yii::app()->functions->FormatDateTime($valt['date_created']);
                            $valt['date_string']=strtotime($valt['date_created']);
                            $trip_data[]=$valt;
                        }
                    } else {
                        $trip_data='';
    ...
    {
        $data='';
        if ($data=Driver::generateTrack($this->data['time_track'],
            $this->data['team_track'], $this->data['driver_track'],
    ...
    public function actiongetMapStatic()
    {
        $this->data=$_GET;
        ...
        if (isset($this->data['id'])){
            ...
            if($trip_details = Driver::getTaskTrip($this->data['id'])){
                ...
                /*foreach ($trip_details as $valt) {
                    ...
                    $oPathPoint->setLatitude($valt['driver_location_lat'])
                        ->setLongitude($valt['driver_location_lng']);
                ...
                $res = '<img src="' . $oStaticMap . '" height="' . $oStaticMap->getHeight() . '" width="' . $oStaticMap->getWidth() . '" />';*/
