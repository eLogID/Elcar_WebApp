...
class AjaxadminController extends CController
{
    ...
    public function actionaddPlans()
    {
    	...
    	if (isset($this->data['id'])){    		
    		...
    		if ( $this->db->updateData("{{plan}}",$params,'plan_id',$this->data['id'] )){
    			$this->msg=t("Successfully updated");
    			$this->code=1;
    		} else $this->msg=t("Something went wrong cannot update records");
    	} else {
    		if ( $this->db->insertData("{{plan}}",$params)){
    			$last_id=Yii::app()->db->getLastInsertID();
    			$this->code=1;
    			$this->msg=t("Successful");
    			$this->details=Yii::app()->createUrl('admin/plan-new',
    			array( 'id'=>$last_id,'msg'=>$this->msg)
    			);
    ...
    {    	
    	$aColumns = array(
		  'plan_id','plan_name','plan_name_description',
		  'plan_type','price','status','plan_id'
		);
		...
		if ( $res=$this->db->rst($stmt)){
			...
			foreach ($res as $val) {			
				...
				$actions=AdminFunctions::generateActionsList('plan_id',$val['plan_id'],'plan','plan-new');
				
				$feed_data['aaData'][]=array(
				  $val['plan_id'],
				  $val['plan_name'],
				  "<span class=\"truncate\">".$val['plan_name_description']."</span>",
				  $val['plan_type'],
				  $val['price']>0?AdminFunctions::prettyPrice($val['price']):'-',
				  $status,
				  $actions
				);
			}
		...
		if ( $res=$this->db->rst($stmt)){
			...
			foreach ($res as $val) {			
				...
				if ($val['status']=="pending" && $val['needs_approval']==1){
				    ...
				    $actions.=AdminFunctions::generateActionsView($val['email_address'], $val['password']);
                }
				
				$actions.=AdminFunctions::generateActionsListCustomer('customer_id',$val['customer_id'],'customer',
				'customer-new');
    ...
    {    	
    	...
    	if (isset($this->data['id'])){    		
    		...
    		if ( $this->db->insertData("{{customer}}",$params)){
    			...
    			$this->code=1;
    			$this->msg=t("Successful");
    			$this->details=Yii::app()->createUrl('admin/customer-new',
    			array( 'id'=>$last_id,'msg'=>$this->msg)
    			);
    		} else $this->msg=t("Something went wrong cannot insert records");
    	}
    ...
    public function actionnewSignup()
    {
		...
		if ( $res=$this->db->rst($stmt)){
			...
			foreach ($res as $val) {			
				...
				$actions.=AdminFunctions::generateActionsList('customer_id',$val['customer_id'],'customer',
				'customer-new');
    ...
    public function actioncurrencyList()
    {
		...
		if ( $res=$this->db->rst($stmt)){
			...
			foreach ($res as $val) {			
				...
				$status="<div class=\"tag $stats\">".t($val['status'])."</div>";
				$status.=AdminFunctions::prettyDate($val['date_created']);
				$actions=AdminFunctions::generateActionsList('curr_id',$val['curr_id'],'currency',
				'currency-new');
    ...
    public function actionaddCurrency()
    {
    	...
    	if (isset($this->data['id'])){    		
    		...
    		if ( $this->db->insertData("{{currency}}",$params)){
    			...
    			$this->code=1;
    			$this->msg=t("Successful");
    			$this->details=Yii::app()->createUrl('admin/currency-new',
    			array( 'id'=>$last_id,'msg'=>$this->msg)
    			);
    		} else $this->msg=t("Something went wrong cannot insert records");
    	}
    ...
    {    	
    	...
    	} else $this->msg=t("Customer id is missing");
    	$this->jsonResponse();
    }
    ...
    public function actioncustomPageList()
    {
		...
		if ( $res=$this->db->rst($stmt)){
			...
			foreach ($res as $val) {			
				...
				$actions=AdminFunctions::generateActionsList('page_id',$val['page_id'],'page',
				'custompage-new');
    ...
    {    	
    	...
    	if (isset($this->data['id'])){    		
    		...
    		if ( $this->db->insertData("{{page}}",$params)){
    			...
    			$this->code=1;
    			$this->msg=t("Successful");
    			$this->details=Yii::app()->createUrl('admin/custompage-new',
    			array( 'id'=>$last_id,'msg'=>$this->msg)
    			);
    		} else $this->msg=t("Something went wrong cannot insert records");
    	}
